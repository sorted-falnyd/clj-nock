...
(defn nock-if
  ...
  Lazy implementation (not compsci lazy, me lazy)
  instead of macro."
  [subject formula]
  (let [b (left formula)
        cd (right formula)
...
(defn nock-push
  "*[a 8 b c]          *[[*[a b] a] c]"
  [subject formula]
  (let [a subject b (left formula) c (right formula)
        s (nock a b)
        s (cell s a)]
    (nock s c)))
...
(defn nock-invoke
  ...
  With c, we produce a noun which contains both code and data
  - a core.
  We use this core as the subject, and evaluate the formula
  within it at slot b.
  [%9 p q] -> [%7 q [%2 [%0 1] [%0 p]]]
  "
  [subject formula]
  ...
  (let [op (left formula)
    ...
    (if (-cell? op)
      ...
      (condp == op
        ...
        const formula ;                          1
        compose (nock-compose subject formula) ; 2
        cell? (nock-cell? subject formula) ;     3
        succ (nock-inc subject formula) ;        4
        eq? (nock-eq? subject formula) ;         5
        branch (nock-if subject formula) ;       6
        then=> (nock-then subject formula) ;     7
        push+= (nock-push subject formula) ;     8
        invoke (nock-invoke subject formula) ;   9
        edit (nock-edit subject formula) ;       10
        hint (nock-hint subject formula)))))

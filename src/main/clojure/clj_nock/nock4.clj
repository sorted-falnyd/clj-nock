...
(deftype Atom [^int value]
  Object
  (equals [_ a]
    (if (instance? Atom a)
      (let [^Atom a a]
        (Numbers/equals value (.-value a)))
      false))
  (toString [_]
    (str value)))

(definline atom? [c] `(instance? Atom ~c))
...
(definline nock-inc
  ...
  `(let [~'a (nock ~subject ~formula)]
     (new Atom (unchecked-inc-int (.value ~(with-meta 'a {:tag "Atom"}))))))
...
(defn nock
  ...
  (let [op (left formula)
    ...
    (if (-cell? op)
      ...
      (case (.value ^Atom op)
        0 (axis (.value ^Atom formula) subject)
        1 formula
        ...
        6 (let [cd (right formula)]
            (recur subject
                   (if (.equals yes (nock subject (left formula)))
                     (left cd)
                     (right cd))))
        ...
        9 (let [core (nock subject (right formula))]
            (recur core (axis (.value ^Atom (left formula)) core)))
